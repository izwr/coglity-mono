...
export const aiGenerationSessions = pgTable('ai_generation_sessions', {
  ...
  followUpQA: jsonb('follow_up_qa')
    .default([])
    .notNull()
    .$type<{ question: string; answer: string }[]>(),
  generatedScenarios: jsonb('generated_scenarios')
    .default([])
    .notNull()
    .$type<{ title: string; description: string }[]>(),
  selectedScenarioIndices: jsonb('selected_scenario_indices')
    .default([])
    .notNull()
    .$type<number[]>(),
...
export const insertAiGenerationSessionSchema = createInsertSchema(aiGenerationSessions, {
  ...
  updatedAt: true,
  followUpQA: true,
  generatedScenarios: true,
  selectedScenarioIndices: true,
  status: true,
});
