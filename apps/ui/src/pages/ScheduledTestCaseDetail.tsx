...
import {
  ...
  type ScheduledTestCaseDetailDTO,
...
export function ScheduledTestCaseDetail() {
  const {
    ...
    caseId,
    projectId,
  } = useParams<{ id: string; caseId: string; projectId: string }>();
  ...
  useSetBreadcrumbs([
    ...
    {
      label: 'Run',
      to: suiteId && projectId ? `/scheduled-test-suites/${projectId}/${suiteId}` : undefined,
    },
    { label: 'Case' },
  ]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await scheduledTestSuiteService.updateCase(
        org.organizationId,
        projectId,
        suiteId,
        caseId,
  ...
  if (!sc)
    return (
      <div className="page-test-suites">
        <p className="ts-empty">Scheduled test case not found.</p>
      </div>
    );
    ...
    <div className="tc-detail">
        ...
        <div className="tc-detail-meta-text">
          ...
          {(editing ? editAssignedTo : sc.assignedTo) && (
            <>
              {' '}
              · Assigned to{' '}
              {allUsers.find((u) => u.id === (editing ? editAssignedTo : sc.assignedTo))
                ?.displayName ?? sc.assignedToName}
            </>
          )}
        </div>
      </div>
        ...
        <div className="tc-detail-section">
          ...
          {editing ? (
            ...
            <div className="tc-detail-content">
              <span
                className="status-badge"
                style={{ color: stateColor, background: `${stateColor}18`, marginLeft: 0 }}
              >
              ...
              value={
                editAssignedTo
                  ? {
                      ...
                      label: allUsers.find((u) => u.id === editAssignedTo)?.displayName ?? '',
                    }
                  : null
              }
            ...
            <div className="tc-detail-content">
              {sc.linkedBugs.length === 0 ? (
                ...
                <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                  {sc.linkedBugs.map((b) => (
                    <span key={b.id} className="bug-badge priority-critical">
                      {b.title}
                    </span>
                  ))}
                </div>
      ...
      <div className="tc-detail-section" style={{ marginTop: '24px' }}>
        ...
        {editing ? (
          ...
          <div className="tc-detail-content tc-detail-content-tall">
            {sc.actualResults || (
              <span className="tc-detail-placeholder">No actual results recorded.</span>
            )}
          </div>
        )}
      </div>
