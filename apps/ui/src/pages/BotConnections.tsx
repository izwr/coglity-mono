...
> = {
  dialin: [{ key: 'phoneNumber', label: 'Phone Number', placeholder: '+1234567890' }],
  websocket: [{ key: 'url', label: 'WebSocket URL', placeholder: 'wss://your-bot.example.com/ws' }],
  http: [
...
const AUDIO_SCHEMA_FIELDS = {
  ...
  outputTemplate: {
    label: 'Output JSON Template (optional)',
    placeholder: '{"event":"media","media":{"payload":""}}',
  },
...
export function BotConnections() {
  ...
  const startEdit = (conn: BotConnectionWithUser) => {
    ...
    setAuthHeaders(
      Array.isArray(savedHeaders) ? (savedHeaders as { key: string; value: string }[]) : [],
    );
    setShowForm(true);
  };
  ...
  return (
    <div className="page wide">
      <PageHead
        title={
          <>
            Bots &amp; <em className="italic-teal">agents</em>
          </>
        }
      ...
      {showForm && (
        <form className="ts-form" onSubmit={handleSubmit(onSubmit)}>
          <div className="ts-form-title">
            {editingId ? 'Edit Bot Connection' : 'Add Bot Connection'}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
            <div className="ts-form-field">
              ...
              <Select
                value={
                  watch('botType')
                    ? { value: watch('botType'), label: botTypeLabel(watch('botType')) }
                    : null
                }
                onChange={(opt) => {
            ...
            <div className="ts-form-field">
              ...
              <Select
                value={
                  watch('provider')
                    ? { value: watch('provider'), label: providerLabel(watch('provider')) }
                    : null
                }
                onChange={(opt) => {
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                <div className="ts-form-field">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setConfigValues((prev) => ({ ...prev, inputAudioField: e.target.value }))
                    }
                  />
                </div>
                <div className="ts-form-field">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setConfigValues((prev) => ({ ...prev, outputAudioField: e.target.value }))
                    }
                  />
                </div>
              </div>
              <div className="ts-form-field">
                ...
                >
                  <Editor
                    ...
                    onChange={(val) =>
                      setConfigValues((prev) => ({ ...prev, outputTemplate: val ?? '' }))
                    }
                    options={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px' }}>
                    ...
                    <Select
                      value={
                        configValues['encoding']
                          ? {
                              value: configValues['encoding'],
                              label:
                                AUDIO_ENCODING_OPTIONS.find(
                                  (o) => o.value === configValues['encoding'],
                                )?.label ?? configValues['encoding'],
                            }
                          : null
                      }
                      onChange={(opt) =>
                        setConfigValues((prev) => ({ ...prev, encoding: opt?.value ?? '' }))
                      }
                  ...
                  <div className="ts-form-field">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setConfigValues((prev) => ({ ...prev, sampleRate: e.target.value }))
                      }
                    />
                  </div>
                  <div className="ts-form-field">
                    ...
                    <Select
                      value={
                        configValues['channels']
                          ? {
                              value: configValues['channels'],
                              label: configValues['channels'] === '1' ? 'Mono (1)' : 'Stereo (2)',
                            }
                          : null
                      }
                      onChange={(opt) =>
                        setConfigValues((prev) => ({ ...prev, channels: opt?.value ?? '' }))
                      }
                      options={[
          ...
          <Tabs
            ...
            options={[
              ...
              },
              {
                value: 'voice',
                label: 'Voice',
                count: connections.filter((c) => c.botType === 'voice').length,
                chipVariant: 'voice',
              },
              {
                value: 'chat',
                label: 'Chat',
                count: connections.filter((c) => c.botType === 'chat').length,
                chipVariant: 'chat',
              },
            ]}
          />
      ...
      {initialLoad ? (
        ...
        <div className="empty">
          <div className="title">
            No <em className="italic-teal">bots</em> connected.
          </div>
          <div className="sub">
            Add a voice or chat bot connection so Coglity can run tests against it.
          </div>
                  ...
                  <tbody>
                    {connections.map((conn) => (
                      <tr key={conn.id}>
                        <td style={{ width: 44 }}>
                          <div
                            style={{
                              ...
                              borderRadius: 8,
                              background:
                                conn.botType === 'voice' ? 'var(--violet-bg)' : 'var(--blue-bg)',
                              display: 'grid',
                              placeItems: 'center',
                              color: conn.botType === 'voice' ? 'var(--violet)' : 'var(--blue)',
                            }}
                          >
                        ...
                        <td>
                          <Chip variant={conn.botType === 'voice' ? 'voice' : 'chat'}>
                            {botTypeLabel(conn.botType)}
                          </Chip>
                        </td>
                        <td className="mono">{providerLabel(conn.provider)}</td>
                        <td>
                          <Chip variant="pass" dot pulse>
                            healthy
                          </Chip>
                        </td>
