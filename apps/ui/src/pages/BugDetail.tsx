...
export function BugDetail() {
  ...
  const handleAddComment = async () => {
    ...
    try {
      if (!projectId || !org) return;
      const updated = await bugService.addComment(
        org.organizationId,
        projectId,
        id,
        commentText.trim(),
  ...
  if (!bug)
    return (
      <div className="page-test-suites">
        <p className="ts-empty">Bug not found.</p>
      </div>
    );
          ...
          {editing ? (
            <Select
              value={{
                ...
                label: RESOLUTION_LABELS[watch('resolution')] ?? '',
              }}
              onChange={(opt) =>
                setValue('resolution', opt?.value ?? 'unresolved', { shouldValidate: true })
              }
              options={Object.entries(RESOLUTION_LABELS).map(([val, label]) => ({
                value: val,
                label,
              }))}
            />
            ...
            <div className="tc-detail-content">
              {RESOLUTION_LABELS[bug.resolution] ?? bug.resolution}
            </div>
          )}
        </div>
        <div className="tc-detail-section">
          ...
          {editing ? (
            <Select
              value={{
                value: watch('reproducibility'),
                label: REPRODUCIBILITY_LABELS[watch('reproducibility')] ?? '',
              }}
              onChange={(opt) =>
                setValue('reproducibility', opt?.value ?? 'always', { shouldValidate: true })
              }
              options={Object.entries(REPRODUCIBILITY_LABELS).map(([val, label]) => ({
                value: val,
                label,
              }))}
            />
            ...
            <div className="tc-detail-content">
              {REPRODUCIBILITY_LABELS[bug.reproducibility] ?? bug.reproducibility}
            </div>
          )}
        </div>
        <div className="tc-detail-section">
      ...
      <div className="tc-detail-section" style={{ marginBottom: '24px' }}>
        ...
        {editing ? (
          ...
          <div className="tc-detail-content tc-detail-content-tall">
            {bug.description || (
              <span className="tc-detail-placeholder">No description provided.</span>
            )}
          </div>
        )}
