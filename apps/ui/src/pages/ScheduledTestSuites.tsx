...
import {
  ...
  type ScheduledTestSuiteListItem,
...
export function ScheduledTestSuites() {
  ...
  useEffect(() => {
    if (!org) return;
    testSuiteService
      .getAll(org.organizationId, projectIds, { limit: 100 })
      .then((res) => setAllSuites(res.data))
      .catch(() => setAllSuites([]));
  }, [org, projectIds]);
  ...
  const isFormValid =
    selectedSuiteId && startDate && endDate && new Date(endDate) >= new Date(startDate);
  ...
  return (
    <div className="page wide">
      <PageHead
        title={
          <>
            <em className="italic-teal">Runs</em> history
          </>
        }
        ...
        <div className="ts-form">
                ...
                value={
                  selectedSuiteId
                    ? {
                        ...
                        label: allSuites.find((s) => s.id === selectedSuiteId)?.name ?? '',
                      }
                    : null
                }
          ...
          <div className="ts-form-actions">
            ...
            <Button
              ...
              disabled={!isFormValid || creating || !formProjectId}
            >
      ...
      {initialLoad ? (
        ...
        <div className="empty">
          <div className="title">
            No <em className="italic-teal">runs</em> yet.
          </div>
          ...
          {chartData.length > 0 && (
            <div className="card" style={{ padding: 20, marginBottom: 16 }}>
              ...
              <div className="sbar" style={{ height: 60 }}>
                {chartData.map((item) => {
                  const pct =
                    item.caseCount > 0 ? Math.round((item.passedCount / item.caseCount) * 100) : 0;
                  const cls =
                    pct >= 95 ? '' : pct >= 85 ? 'amber' : item.caseCount === 0 ? 'muted' : 'red';
                  return (
                    <span
                      key={item.id}
                      className={cls}
                      style={{ height: `${Math.max(10, pct)}%` }}
                      title={`${item.testSuiteName} · ${pct}%`}
                    />
                  );
                })}
              </div>
                ...
                <table className="t">
                  ...
                  <tbody>
                    {items.map((item) => {
                      const pct =
                        item.caseCount > 0
                          ? Math.round((item.passedCount / item.caseCount) * 100)
                          : 0;
                      const status =
                        item.failedCount === 0 &&
                        item.passedCount === item.caseCount &&
                        item.caseCount > 0
                          ? 'passed'
                          : item.failedCount > 0
                            ? 'failed'
                      ...
                      return (
                        <tr
                          key={item.id}
                          onClick={() =>
                            navigate(`/scheduled-test-suites/${item.projectId}/${item.id}`)
                          }
                            ...
                            <div className="row" style={{ gap: 8 }}>
                              <div className="progress" style={{ flex: 1, maxWidth: 120 }}>
                                ...
                                <span
                                  className="fail"
                                  style={{
                                    width: `${item.failedCount && item.caseCount ? (item.failedCount / item.caseCount) * 100 : 0}%`,
                                  }}
                                />
                              </div>
