...
export function GenerateTestCases() {
  ...
  useEffect(() => {
    ...
    testSuiteService
      .getAll(org.organizationId, ids, { limit: 100 })
      .then((res) => {
  ...
  const handleCreateSession = async () => {
    ...
    try {
      ...
      const qs = await aiService.getFollowUpQuestions(
        org.organizationId,
        formProjectId,
        session.id,
      );
  ...
  const handleSubmitAnswers = async () => {
    ...
    try {
      ...
      const updated = await aiService.generateScenarios(
        org.organizationId,
        formProjectId,
        sessionId,
      );
  ...
  const handleCreateTestCases = async () => {
    ...
    setError('');
    try {
      await aiService.createTestCases(
        org.organizationId,
        formProjectId,
        sessionId,
  ...
  return (
    <div className="page narrow">
      ...
      <PageHead
        title={
          <>
            Generate <em className="italic-teal">cases</em> from a story
          </>
        }
      ...
      <div className="ai-stepper">
        {stepLabels.map((label, i) => (
          <div
            key={label}
            className={`ai-step${i <= currentStepIndex ? ' ai-step-active' : ''}${i < currentStepIndex ? ' ai-step-completed' : ''}`}
          >
              ...
              <Select
                value={
                  testSuiteId
                    ? {
                        value: testSuiteId,
                        label: allSuites.find((s) => s.id === testSuiteId)?.name ?? '',
                      }
                    : null
                }
      ...
      {step === 'followup' && (
        <div className="ts-form">
          ...
          <p className="ts-form-hint" style={{ marginBottom: '16px' }}>
            The AI has some questions to help generate better test scenarios. Answer as many as you
            can.
          </p>
          {questions.map((q, i) => (
        ...
        <div>
          ...
          <div className="ts-form-actions" style={{ marginTop: '20px' }}>
            ...
            >
              {loading
                ? 'Creating...'
                : `Create ${selectedIndices.size} Test Case${selectedIndices.size !== 1 ? 's' : ''} as Draft`}
            </Button>
          </div>
        </div>
      )}
      ...
      {step === 'done' && (
        <div className="ts-empty-state">
          ...
          <span style={{ marginBottom: '20px' }}>
            Review them in the test cases list and mark them active when ready.
          </span>
          <Button onClick={() => navigate('/test-cases')}>View Test Cases</Button>
        </div>
      )}
    </div>
  );
