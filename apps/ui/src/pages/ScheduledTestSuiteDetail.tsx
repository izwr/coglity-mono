...
import {
  ...
  type ScheduledTestSuiteDetail as SuiteDetail,
} from '../services/scheduledTestSuiteService';
...
export function ScheduledTestSuiteDetail() {
  ...
  if (!suite)
    return (
      <div className="page">
        <p className="ts-empty">Not found.</p>
      </div>
    );

  const pct = suite.caseCount > 0 ? Math.round((suite.passedCount / suite.caseCount) * 100) : 0;
  const failedPct =
    suite.caseCount > 0 ? Math.round((suite.failedCount / suite.caseCount) * 100) : 0;
  ...
  const status =
    runningCount > 0
      ...
      : suite.failedCount > 0
        ? 'failed'
        : suite.passedCount === suite.caseCount && suite.caseCount > 0
          ? 'passed'
          : 'idle';
  ...
  return (
    <div className="page wide">
      <Button
        ...
        onClick={() => navigate('/scheduled-test-suites')}
        ...
        <div className="page-head-text">
          <div className="row" style={{ marginBottom: 10 }}>
            ...
            <span className="mono muted" style={{ fontSize: 12 }}>
              run · {suite.id.slice(0, 8)}
            </span>
          </div>
          <h1>
            <em className="italic-teal">{suite.testSuiteName}</em>
          </h1>
          <div className="sub">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18 }}>
          <div>
            ...
            <div className="muted" style={{ fontSize: 12 }}>
              {suite.passedCount}/{suite.caseCount} cases
            </div>
          </div>
          <div>
            ...
            <div
              ...
              style={{ fontSize: 28, color: suite.failedCount > 0 ? 'var(--red)' : 'var(--ink)' }}
            >
              {suite.failedCount}
            </div>
            <div className="muted" style={{ fontSize: 12 }}>
              ...
              options={[
                {
                  value: 'all',
                  label: 'All',
                  count: suite.scheduledCases.length,
                  chipVariant: 'teal',
                  dotColor: 'var(--teal)',
                },
                {
                  value: 'failed',
                  label: 'Failed',
                  count: suite.scheduledCases.filter(
              ...
              cases.map((sc) => (
                <button
                  ...
                  style={{
                    ...
                    borderLeft:
                      selectedCaseId === sc.id ? '3px solid var(--teal)' : '3px solid transparent',
                    cursor: 'pointer',
                  }}
                >
                  ...
                  <div
                    style={{
                      fontSize: 13,
                      color: 'var(--ink)',
                      fontWeight: selectedCaseId === sc.id ? 500 : 400,
                      lineHeight: 1.4,
                    }}
                  >
                    {sc.title ?? 'Unknown'}
                ...
                <div className="actions">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      id &&
                      projectId &&
                      navigate(`/scheduled-test-suites/${projectId}/${id}/cases/${selectedCase.id}`)
                    }
                  >
                    Open detail
                  </Button>
              ...
              <div className="card-body">
                <div className="row gap-lg" style={{ flexWrap: 'wrap', marginBottom: 18 }}>
                  <div>
                    ...
                    <Chip
                      variant={STATE_CHIP[selectedCase.state]}
                      dot
                      pulse={selectedCase.state === 'in_progress'}
                    >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 13 }}>
                      {selectedCase.assignedToName ?? <span className="muted">Unassigned</span>}
                    </div>
                  </div>
                  <div>
                    <div className="section-label" style={{ marginBottom: 4 }}>
                    ...
                    <div className="mono" style={{ fontSize: 13 }}>
                      {selectedCase.linkedBugs.length}
                    </div>
                  </div>
                </div>
                ...
                {selectedCase.state === 'failed' && (
                  ...
                  >
                    ...
                    <p
                      style={{ fontSize: 14, color: 'var(--ink)', marginTop: 6, lineHeight: 1.55 }}
                    >
                      Expected bot turn containing{' '}
                      <code
                        style={{
                          fontFamily: 'var(--mono)',
                          fontSize: 12,
                      ...
                      {`Bot: Here is your confirmation number.
{
  ...
  "value": "A-7812"
                  ...
                  <div className="col">
                    ...
                    <pre
                      style={{
                        ...
                        fontSize: 12,
                        background:
                          selectedCase.state === 'failed' ? 'var(--red-bg)' : 'var(--bg-2)',
                        padding: 12,
                        borderRadius: 8,
                        border: `1px solid ${selectedCase.state === 'failed' ? 'var(--red-line)' : 'var(--line)'}`,
                        overflow: 'auto',
                        margin: 0,
                      }}
                    >
            ...
            <div className="empty">
              <div className="title">
                Select a <em className="italic-teal">case</em>
              </div>
              <div className="sub">
                Pick a case from the left to see its transcript, tool calls and telemetry.
              </div>
            </div>
          )}
        </div>
