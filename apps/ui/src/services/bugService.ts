...
export const bugService = {
  ...
  ): Promise<PaginatedResponse<BugWithDetails>> {
  ...
  async getById(orgId: string, projectId: string, id: string): Promise<BugWithDetails> {
    const { data } = await api.get<BugWithDetails>(
      `/organizations/${orgId}/projects/${projectId}/bugs/${id}`,
    );
    return data;
  },
  ...
  ): Promise<BugWithDetails> {
    const { data } = await api.post<BugWithDetails>(
  ...
  async update(
    ...
    const { data } = await api.put<BugWithDetails>(
      `/organizations/${orgId}/projects/${projectId}/bugs/${id}`,
      payload,
    );
    return data;
  ...
  async addComment(
    ...
    const { data } = await api.post<BugWithDetails>(
      `/organizations/${orgId}/projects/${projectId}/bugs/${id}/comments`,
      { text },
    );
    return data;
  },
