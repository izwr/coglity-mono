...
const base = (orgId: string, projectId: string) =>
  `/organizations/${orgId}/projects/${projectId}/ai`;
...
export const aiService = {
  async createSession(
    ...
    const { data } = await api.post<AiGenerationSession>(`${base(orgId, projectId)}/session`, {
      testSuiteId,
      userStory,
    });
    return data;
  ...
  async getFollowUpQuestions(
    ...
    const { data } = await api.post<{ questions: string[] }>(
      `${base(orgId, projectId)}/session/${sessionId}/followup`,
    );
  ...
  async submitAnswers(
    ...
    const { data } = await api.post<AiGenerationSession>(
      `${base(orgId, projectId)}/session/${sessionId}/answers`,
      { answers },
    );
    return data;
  },
  ...
  async generateScenarios(
    ...
    const { data } = await api.post<AiGenerationSession>(
      `${base(orgId, projectId)}/session/${sessionId}/generate-scenarios`,
    );
    return data;
  },
  ...
  async createTestCases(
    ...
    const { data } = await api.post<unknown[]>(
      `${base(orgId, projectId)}/session/${sessionId}/create-test-cases`,
      { selectedIndices },
    );
    return data;
  },
