...
export const tagService = {
  ...
  async getById(orgId: string, projectId: string, id: string): Promise<TagWithUsers> {
    const { data } = await api.get<TagWithUsers>(
      `/organizations/${orgId}/projects/${projectId}/tags/${id}`,
    );
    return data;
  },

  async create(orgId: string, projectId: string, payload: CreateTagPayload): Promise<TagWithUsers> {
    const { data } = await api.post<TagWithUsers>(
  ...
  ): Promise<TagWithUsers> {
    const { data } = await api.put<TagWithUsers>(
      `/organizations/${orgId}/projects/${projectId}/tags/${id}`,
      payload,
    );
    return data;
