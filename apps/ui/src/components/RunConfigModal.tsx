...
export function RunConfigModal({ onSubmit, onCancel, submitting }: Props) {
  ...
  const [combinations, setCombinations] = useState<
    Array<{ language: string; environment: string }>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <label style={radioLabelStyle}>
                ...
                <span className="muted" style={{ fontSize: 11 }}>
                  {' '}
                  — every language &times; every environment
                </span>
              </label>
              <label style={radioLabelStyle}>
                <input
          ...
          {mode === 'mixMatch' && (
            <div style={sectionStyle}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {combinations.map((combo, idx) => (
                  <div key={idx} style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <div style={{ flex: 1 }}>
                      <Select
                        value={{
                          value: combo.language,
                          label:
                            SUPPORTED_LANGUAGES.find((l) => l.code === combo.language)?.label ??
                            combo.language,
                        }}
                        onChange={(opt) =>
                          updateCombination(idx, 'language', (opt as { value: string }).value)
                        }
                    ...
                    <div style={{ flex: 1 }}>
                      <Select
                        value={{
                          value: combo.environment,
                          label:
                            SUPPORTED_ENVIRONMENTS.find((e) => e.id === combo.environment)?.label ??
                            combo.environment,
                        }}
                        onChange={(opt) =>
                          updateCombination(idx, 'environment', (opt as { value: string }).value)
                        }
                        options={SUPPORTED_ENVIRONMENTS.map((e) => ({
                          value: e.id,
                          label: e.label,
                        }))}
                      />
                ...
                <Button variant="ghost" size="sm" onClick={addCombination}>
                  + Add Combination
                </Button>
              </div>
            </div>
          ...
          <div style={summaryStyle}>
            ...
            {mode === 'crossProduct' && runCount > 1 && (
              <span className="muted">
                ...
                &times; {selectedEnvironments.length} environment
