...
const workspace: NavItemDef[] = [
  ...
  { to: '/test-suites', label: 'Suites', d: 'M4 5h6v6H4zM14 5h6v6h-6zM4 15h6v4H4zM14 15h6v4h-6z' },
  {
    to: '/bot-connections',
    label: 'Bots & Agents',
    d: 'M12 4v3M6 9h12a2 2 0 012 2v7a2 2 0 01-2 2H6a2 2 0 01-2-2v-7a2 2 0 012-2zM9 14h.01M15 14h.01',
  },
];
...
const testing: NavItemDef[] = [
  { to: '/test-cases', label: 'Test cases', d: 'M4 6h16M4 12h16M4 18h10' },
  {
    ...
    label: 'New test case',
    d: 'M4 20l4-1 10-10a2.5 2.5 0 00-3.5-3.5L4.5 15.5 4 20z',
  },
  { to: '/scheduled-test-suites', label: 'Runs', d: 'M5 3l14 9-14 9V3z' },
  {
    to: '/bugs',
    label: 'Bugs',
    d: 'M12 2a6 6 0 00-6 6v2a6 6 0 0012 0V8a6 6 0 00-6-6zM4 12h16M4 17h16M12 8v10',
  },
...
function NavItem({ item }: { item: NavItemDef }) {
  ...
  const isActive = (() => {
    ...
    if (item.to === '/test-cases')
      return (
        pathname === '/test-cases' ||
        (pathname.startsWith('/test-cases/') && pathname !== '/test-cases/generate')
      );
    ...
    <>
      ...
      {org && canAdminAnyProject && (
        <>
          ...
          <NavLink
            to={`/orgs/${org.organizationId}/projects`}
...
function OrgNav() {
  ...
  return (
    <>
      ...
      >
        <svg className="ico" viewBox="0 0 24 24">
          <path d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H2v-2a4 4 0 013-3.87M16 3.13a4 4 0 010 7.75M8 3.13a4 4 0 010 7.75M12 14a4 4 0 100-8 4 4 0 000 8z" />
        </svg>
    ...
    <div className="profile-dropdown" ref={ref}>
      ...
      >
        ...
        <div className="meta">
          ...
          <div className="s">
            {user.email.split('@')[0]} · {org?.orgRole ?? 'member'}
          </div>
        </div>
        <svg className="ico chev" viewBox="0 0 24 24" width="14" height="14">
        ...
        <div className="profile-menu" role="menu">
          ...
          {org && org.orgRole === 'super_admin' && (
            <button
              ...
              onClick={() => go(`/orgs/${org.organizationId}/settings`)}
            >
              <svg className="ico" viewBox="0 0 24 24">
...
export function Sidebar({
  ...
  const title =
    variant === 'preferences' ? 'Preferences' : variant === 'org' ? 'Organization' : null;
  ...
  return (
    <aside className={`sidebar${open ? ' open' : ''}`} onClick={() => onClose?.()}>
      ...
      {title && <div className="sidebar-subtitle">{title}</div>}
      ...
      {variant === 'workspace' && user && user.organizations.length > 0 && (
        <div className="sidebar-switchers" onClick={(e) => e.stopPropagation()}>
          <Select
            ...
            options={user.organizations.map((m) => ({
