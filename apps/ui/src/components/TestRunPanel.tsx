...
export function TestRunPanel({ orgId, projectId, runId, onTerminal }: Props) {
  ...
  return (
    <div className="run-panel" style={panelStyle}>
      ...
      >
        <Chip variant={chip.variant} dot pulse={chip.pulse}>
          {chip.label}
        </Chip>
        {run.language && run.language !== 'en-US' && (
          <Chip variant="info">
            {SUPPORTED_LANGUAGES.find((l) => l.code === run.language)?.label ?? run.language}
          </Chip>
        )}
        {run.environment && run.environment !== 'quiet' && (
          <Chip variant="warn">
            {SUPPORTED_ENVIRONMENTS.find((e) => e.id === run.environment)?.label ?? run.environment}
          </Chip>
        )}
        {run.startedAt && (
