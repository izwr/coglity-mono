...
export async function runVoiceTest(payload: RunPayload): Promise<void> {
  ...
  const finishWithProps = async (
    ...
    console.log(
      `${TAG} finishing: state=${state}${opts.error ? ` error="${opts.error}"` : ''}${opts.verdict ? ` verdict="${opts.verdict.slice(0, 120)}"` : ''}`,
    );
  ...
  try {
    ...
    await finishWithProps('errored', {
      error: `azure auth failed: ${err instanceof Error ? err.message : String(err)}`,
    });
    return;
  }
  ...
  speechConfig.speechSynthesisVoiceName =
    payload.ttsVoice ?? process.env.AZURE_SPEECH_VOICE ?? 'en-US-AvaMultilingualNeural';
  ...
  if (!aoaiEndpoint) {
    await finishWithProps('errored', { error: 'AZURE_OPENAI_ENDPOINT not set' });
    return;
  }
  ...
  if (!sutUrl) {
    await finishWithProps('errored', { error: 'bot_connection.config.url is empty' });
    return;
  }
  ...
  try {
    ...
    await finishWithProps('errored', {
      error: `sut connect failed: ${err instanceof Error ? err.message : String(err)}`,
    });
    return;
  }
  ...
  timeoutHandle = setTimeout(() => {
    computeAndFinish('errored', { error: `timeout: exceeded ${maxDurationMs}ms` });
  }, maxDurationMs);
  ...
  const computeAndFinish = async (
    state: 'passed' | 'failed' | 'errored',
    opts: { verdict?: string; error?: string },
  ) => {
    clearTimeout(timeoutHandle);
    const totalDurationMs = Date.now() - startedAt.getTime();

    // Compute rule-based metrics
    const ruleMetrics = computeRuleBasedMetrics(
      transcript,
      turnTimings,
      totalDurationMs,
      sampleRate,
    ...
    if (state !== 'errored' && transcript.length > 0) {
      try {
        ...
        llmMetrics = await evaluateWithLlm(
          ...
          aoaiDeployment,
          transcript,
          payload.testCase.testSteps,
          payload.testCase.expectedResults,
        );
  ...
  try {
    for (let turn = 0; turn < maxTurns && !finished; turn++) {
      ...
      if (gapMs > 100) {
        audioSegments.push({
          side: 'silence',
          pcm: Buffer.alloc(Math.round((sampleRate * 2 * gapMs) / 1000)),
        });
      }
      ...
      if (toolCall?.function.name === 'submit_verdict') {
        ...
        console.log(
          `${TAG} verdict=${args.verdict} (LLM ${timing.llmMs}ms) reasoning="${(args.reasoning ?? '').slice(0, 200)}"`,
        );
        turnTimings.push(timing);
        await computeAndFinish(args.verdict === 'pass' ? 'passed' : 'failed', {
          verdict: args.reasoning,
        });
        return;
      }
      ...
      if (testerText && sut.readyState === WebSocket.OPEN) {
        ...
        if (gapMs2 > 100) {
          audioSegments.push({
            side: 'silence',
            pcm: Buffer.alloc(Math.round((sampleRate * 2 * gapMs2) / 1000)),
          });
        }
        ...
        for (
          let offset = 0;
          offset < audioToSend.length && sut.readyState === WebSocket.OPEN;
          offset += CHUNK_SIZE
        ) {
    ...
    clearTimeout(timeoutHandle);
    await finishWithProps('errored', {
      error: `run error: ${err instanceof Error ? err.message : String(err)}`,
    });
  }
}
...
function collectSutAudio(sut: WebSocket, cfg: BotConnectionConfig): Promise<Buffer | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    recognizer.startContinuousRecognitionAsync(
      ...
      (err) => {
        clearTimeout(timeout);
        recognizer.close();
        reject(new Error(`STT start failed: ${err}`));
      },
    );
  });
}
...
function synthesizeSpeech(config: speechSdk.SpeechConfig, text: string): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const synthesizer = new speechSdk.SpeechSynthesizer(
      config,
      undefined as unknown as speechSdk.AudioConfig,
    );
    synthesizer.speakTextAsync(
      ...
      (result) => {
        ...
        if (result.reason === speechSdk.ResultReason.SynthesizingAudioCompleted) {
          ...
          reject(
            new Error(
              `TTS failed: ${speechSdk.ResultReason[result.reason]} — ${result.errorDetails ?? ''}`,
            ),
          );
        }
      },
