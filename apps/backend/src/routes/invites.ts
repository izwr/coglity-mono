...
import { auditRbac, createInvite } from '../services/rbac';
...
router.post('/', requireAuth, resolveOrg, requireOrgRole('super_admin'), async (req, res) => {
  ...
  const [proj] = await db
    ...
    .from(projects)
    .where(
      and(eq(projects.id, parsed.data.projectId), eq(projects.organizationId, req.organizationId!)),
    )
    .limit(1);
  if (!proj) {
    ...
    const [updated] = await db
      ...
      .where(
        and(
          ...
          eq(invites.organizationId, req.organizationId!),
